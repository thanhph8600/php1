...
if (isset($_GET['id']) && $_GET['id']) {
    $id = $_GET['id'];
    $sqlId = "SELECT * FROM `banner` WHERE `id` = '" . $id . "'";
    $item = executeResult($sqlId);
    $slogan = $item[0]['slogan'];
    $img = $item[0]['thumb'];
}
...
if (isset($_POST['addbanner']) && $_POST['addbanner']) {
    ...
    $rand = rand(1,9999);
    $nameFile =$rand. $_FILES["upload"]["name"];
    echo $name . $date . $nameFile;
    //check file
    //file
    $target_dir = "uploads/";
    ...
    if (empty($id)) {
        $sql = "INSERT INTO `banner` (`id`, `thumb`, `slogan`, `created_at`, `update_at`) VALUES (NULL, '".$nameFile."', '".$name."', '" . $date . "', '" . $date . "')";
        execute($sql);
        header('location: ./banner.php');
    } else {
        $sql = "UPDATE `banner` SET `slogan` = '".$name."',`thumb` = '".$nameFile."' WHERE `banner`.`id` = '".$id."'";
        execute($sql);
        header('location: ./banner.php');
    }
}
                ...
                <div class="col-12">
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            ...
                            <div class="pst-ab">
                                <a href="./banner.php"><i class="fa fa-arrow-circle-o-left" aria-hidden="true"></i></a>
                            </div>
                        </div>
                        <div class="card-body px-0 pb-2">
                            <div class="table-responsive p-0">
                                <form action="" enctype="multipart/form-data" method="post" onsubmit="return checkproduct()" style="display:flex;width:95%;margin:auto">
                                    ...
                                    <input name="slogan" type="text" id="nameproduct" value="<?=$slogan?>" onkeydown="checkproduct()" onkeypress="checkproduct()" onkeyup="checkproduct()">

                                    <label for="">Thumb</label>
                                    <input type="file" name="upload" id="file-input" accept="image/*">
                                    <div class="img-product">
                                        <img style="display: block;" src="./uploads/<?=$img?>" alt="" id="img-preview">
                                    </div>

                                    <label style="color:red;" for=""></label>
                                    <input name="addbanner" type="submit" value="Add">
                                </form>
                            </div>
                        </div>
                    </div>
